...
./node_modules/.bin/ndjson-map 'd[0].properties = {registro: Number(d[1].V002.replace(",", "."))}, d[0]' < pb-registro.ndjson > pb-ortho-comdado.ndjson
...
./node_modules/.bin/topo2geo tracts=- < pb-quantized-topo.json | ./node_modules/.bin/ndjson-map -r d3 'z = d3.scaleSequential(d3.interpolateViridis).domain([0, 1e3]), d.features.forEach(f => f.properties.fill = z(f.properties.registro)), d' | ./node_modules/.bin/ndjson-split 'd.features' | ./node_modules/.bin/geo2svg -n --stroke none -w 1000 -h 600 > pb-tracts-threshold-light.svg
